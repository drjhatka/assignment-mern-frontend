...
const ShowAllBikes = () => {
    ...
    const handleFilters = (event:React.FormEvent<HTMLFormElement>)=>{
        ...
        const formData = event.currentTarget.elements as HTMLFormControlsCollection
        const inputData = [
            (formData.namedItem('category') as HTMLInputElement)?.value,
            (formData.namedItem('brand') as HTMLInputElement)?.value,
        ...
        setBikeData(
            data.data.filter((bike:Bike)=>{return (
                inputData[0]=='all'|| bike.category===inputData[0] as keyof BikeTypes &&
